...
format: 
  live-revealjs: 
    output-file: 13-dplyr-2-slides.html
    df-print: default
webr:
  cell-options:
    autorun: false
...
::: lab
[ **16-Joining tables** ]{style="color:blue"}
...
```{python}
## dplyr::left_join()
print(pop_x.merge(right=elec_vote_y, 
                  how='left', on='state'))
```
:::
::::
...
## dplyr::right_join()
print(pop_x.merge(elec_vote_y, how = 'right'))
```
:::
...
#| eval: true
## dplyr::full_join()
print(pop_x.merge(elec_vote_y, how = 'outer'))
```
:::
...
#| eval: true
## dplyr::inner_join()
print(pop_x.merge(elec_vote_y, how = 'inner'))
```
:::
