...
format: 
  live-revealjs: 
    output-file: 17-logistic-reg-slides.html
    df-print: default
webr:
  cell-options:
    autorun: false
...
# Logistic function
logistic_fun <- function(x, beta_0, beta_1) {
...
param_tbl <- tribble(
  ...
  mutate(
    curve = paste0("beta[0] == ", beta_0, " * ', ' ~ beta[1] == ", beta_1),
...
plot_tbl <- expand_grid(
  ...
  mutate(
    p = logistic_fun(x, beta_0, beta_1)
  )

# Plot
ggplot(plot_tbl, aes(x = x, y = p, color = curve)) +
  geom_line(linewidth = 1) +
  scale_color_discrete(labels = scales::label_parse()) +
  labs(
    title = expression("Logistic Curves for Different " * beta[0] * " and " * beta[1]),
    x = "x",
    y = expression(P(Y == 1 ~ "|" ~ x)),
    color = "Parameters"
  ) +
  theme_minimal() +
  theme(
...
## Simple Binary Logistic Regression Model {visibility="hidden"}
