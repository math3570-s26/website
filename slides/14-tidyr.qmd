...
format: 
  live-revealjs: 
    output-file: 14-tidyr-slides.html
    df-print: default
webr:
  cell-options:
    autorun: false
...
-   The new `purchases` data set and the `prices` data can now be joined together with the common key variable `item`. <!-- - With the joined data set, we can further calculate total revenue for example. -->
...
```{python}
print(purchases)
print(purchases.pivot(index = "customer_id", columns = "item_no", values = "item"))
```
