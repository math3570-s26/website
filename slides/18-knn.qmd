...
format: 
  live-revealjs: 
    output-file: 18-knn-slides.html
    df-print: default
webr:
  cell-options:
    autorun: false
...
&= \frac{1}{3}
\end{align} <!-- Thus --> <!-- $\hat{C}_3(x = (8, 6)) = \text{Blue}$ -->
:::
:::::
...
-   In the binary case this becomes $$\hat{C}_K(x) = \begin{cases}
      0      & p_{Km}(x) < 0.5\\
      1      & p_{Km}(x) > 0.5
    \end{cases}.$$ and if the probability for class $0$ and $1$ are equal, simply assign at random.
...
::: notes
A recipe is a description of the steps to be applied to a data set in order to prepare it for data analysis. feature engineering steps to get your data ready for modeling
...
::: notes
knn_recipe \<- recipes::recipe(GENDER \~ HEIGHT, data = df_trn) \|\> step_scale(all_predictors()) \|\> step_center(all_predictors())
...
::: lab
[ **22-K Nearest Neighbors** ]{style="color:blue"}
